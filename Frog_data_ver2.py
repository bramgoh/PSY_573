...
#%%
spin = pd.read_csv("spin_by_item.tsv", sep = "\t")
spin = spin[spin["TrialNum"].notna()]      # Dropping empty rows
spin = spin[["spin_sentence_correct", "Predictability", "qualtrics_id"]]
spin_grouped = spin.groupby(["qualtrics_id", "Predictability"]).agg(
    no_of_trials = pd.NamedAgg(column = "spin_sentence_correct", aggfunc = len),
    no_of_successes = pd.NamedAgg(column = "spin_sentence_correct", aggfunc = "sum"))
spin_wide = spin_grouped.pivot_table(index = "qualtrics_id", columns = "Predictability", values = ["no_of_trials", "no_of_successes"])
spin_wide = spin_wide.droplevel(0, axis = 1)
spin_wide.columns = ["Highspin_successes", "Lowspin_successes", "Highspin_trials", "Lowspin_trials"]

span = pd.read_csv("span_by_item.tsv", sep = "\t")
span = span[span["qualtrics_id"].notna()]
span = span[span["Set"] != "Practice"]
span = span[["sent_correct", "qualtrics_id"]]
span_grouped = span.groupby("qualtrics_id").agg(
    Span_trials = pd.NamedAgg(column = "sent_correct", aggfunc = len),
    Span_successes = pd.NamedAgg(column = "sent_correct", aggfunc = "sum"))

demo = pd.read_csv("demospinspan.csv")
demo = demo[["qualtrics_id", "SES.ycoord", "political_scale", "political_interest", "schooling_level", "age"]]
frog_full = reduced_data.merge(spin_wide, how = "left", on = "qualtrics_id").merge(span_grouped, how = "left", on = "qualtrics_id").merge(demo, how = "left", on = "qualtrics_id")
frog_full = frog_full[frog_full["Highspin_successes"].notnull()]
frog_full = frog_full[frog_full["Span_successes"].notnull()]
frog_reduced = frog_full[["qualtrics_id", "word_count", "bigram_count", "trigram_count", "combination_ratio", "content_word_count", "content_word_ratio", 
                          "median_SUBTLEX_freq", "log_min_SUBTLEX_freq", "SES.ycoord", "political_scale", "political_interest", "schooling_level", "age", 
                          "ttr", "Highspin_successes", "Lowspin_successes", "Highspin_trials", "Lowspin_trials", "Span_trials", "Span_successes"]]
